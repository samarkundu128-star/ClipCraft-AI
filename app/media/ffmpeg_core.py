...
# --- 1. Fake HTTP Server for Render Free Tier ---
...
async def process_video_pipeline(msg, input_video_path: str, message_id: int, update: Update):
    # FIXED: Hardcoded .mp3 ki jagah .wav use ho raha hai
    ...
    try:
        ...
        await msg.edit_text("🎧 **Step 1/5:** Audio extract ho raha hai...", parse_mode="Markdown")
        try:
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        ...
        "📹 Mujhe koi bhi video (MP4) ya **YouTube Link** bhejo, main Gemini + Whisper use karke auto viral Shorts render kar doonga.",
        parse_mode="Markdown"
    )


async def handle_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = await update.message.reply_text("📥 **Video file receive hui, process start ho rahi hai...**", parse_mode="Markdown")
    ...
    async def run_task():
        try:
            ...
            if "File is too big" in str(e):
                await msg.edit_text("❌ **Telegram Limit:** File 20MB se badi hai. Direct YouTube/Reels link bhejein!")
            else:
                await msg.edit_text(f"❌ Video file download error: `{str(e)}`", parse_mode="Markdown")
        ...
        async def run_task():
            try:
                ...
                if not os.path.exists(input_video_path):
                    raise Exception("Video download complete nahi hui.")
                await process_video_pipeline(msg, input_video_path, message_id, update)
            except Exception as e:
